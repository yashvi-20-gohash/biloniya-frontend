...
interface DataTableRowActionsProps<TData extends TourListType> {
  row: CustomRow<TData>
}

export function DataTableRowActions<TData extends TourListType>({
  row,
  ...
  const handleConfirmDelete = async () => {
    const resp = await deleteTour(id)
  ...
  const handleUpdateState = async () => {
    ...
    const resp = await updateTourStatus(data)
    console.log("resp", resp);

    if (resp?.status === 'SUCCESS') {
      const data = { ...row.original, ...resp?.data?.lastData as TourListType }
