...
export const AddModel: React.FC<{
  ...
  addRow: (newRow: TourListType) => void
}> = ({ openStatusDialog, setOpenStatusDialog, addRow }) => {
  const { addTourLoading, addTour } = SuperAdminTourStore()
  ...
  const onSubmit = async (data: TourFormValue) => {
    ...
    try {
      ...
      if (response?.status === 'SUCCESS') {
        const addedData = response?.data?.lastData as TourListType
        if (addedData) {
          addRow(addedData)
        }
        reset()
  ...
  return (
    <Dialog open={openStatusDialog} onOpenChange={closeStatusDialog}>
      <DialogContent className="sm:max-w-[1000px] max-h-[80%] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-bold">Add Tour Package</DialogTitle>
        </DialogHeader>
