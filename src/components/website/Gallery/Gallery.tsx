...
const Gallery = () => {
  const { getGalleryAllList, allGalleryData, isAllGalleryLoading } = userGalleryListStore();
  ...
  useEffect(() => {
    ...
    // Check if galleryContainer is not null
  ...
  return (
    <div className="py-4">
      <div className="header flex flex-col items-center mt-3">
        <h5 className="text-3xl font-semibold mb-3">
          <span className="flex items-center text-xl gap-4 font-secound text-[#b0191e]">
            <svg
      ...
      <div
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 px-4 py-6 max-w-screen-xl mx-auto"
      >

        {
          isAllGalleryLoading && allGalleryData.length <= 0 ? (
            Array.from({ length: 4 }).map((_, index) => (
              <>
                <div key={index}>
                  <Skeleton className="w-52 h-52 rounded-sm mx-auto mb-2" />
                </div>
              </>
            ))
          ) : (
            allGalleryData.length > 0 ? (
              allGalleryData.map((image, index) => (
                // <Link key={index} href={image?.imageUrl}>
                <>
                  <div
                    id="gallery-container"
                    key={index}>
                    <Image
                      src={image?.imageUrl || '/gallery/hotel-3.jpg'}
                      alt={image?.image}
