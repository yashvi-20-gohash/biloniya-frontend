...
import { userTourPackageStore } from '@/src/store/website/tour-package-management';
import { ServiceSkeleton } from '../../skeleton/service';
import NoDataFound from '../../NoDataFound';


const ContactSection = () => {
  const { getTourList, tourData, isTourLoading } = userTourPackageStore()
  ...
  return (
    <div className=" px-4 py-16 affodable-section">
      <div className='max-w-screen-xl mx-auto'>
        <div className="text-center mb-8">
          <div className="flex items-center font-secound justify-center gap-2 mb-1">
            ...
            <span className='text-[#b0191e]'>Tour Package</span>
            <span className="text-xl"><svg
              xmlns="http://www.w3.org/2000/svg"
              width="15"
              height="16"
          ...
          <h2 className="lg:text-[42px] text-2xl font-bold">Affordable Vacation Bundles</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7 mb-10">
          {isTourLoading ? (
            ...
            tourData?.length > 0 ? (
              tourData?.map((tour) => (
                <div key={tour?._id} className="bg-white rounded-lg relative shadow-[0_2px_8px_0px_rgba(0,0,0,0.2)] overflow-hidden p-4">
                  <div >
                    {tour?.tourImageUrl && (
                      <Image
                  ...
                  <div className="pt-6 pb-2">
                    ...
                    <div className="flex gap-1.5 items-center mb-4 overflow-x-auto max-w-full scrollbar-hide">
                      {tour?.tags.map((tags, index) => (
                        <React.Fragment key={tags}>
                          <span className="text-gray-600 text-sm whitespace-nowrap">{tags}</span>
        ...
        {tourData.length > 0 ? (
          <div className="text-center">
            <button className="inline-flex items-center gap-2 px-9 py-3 rounded text-white bg-[#b0191e] hover:bg-black text-title-color text-base font-medium tracking-wide capitalize leading-tight transition duration-500 relative overflow-hidden z-10 whitespace-nowrap hover:text-white">
              View All Package
            </button>
          </div>
        ) :
