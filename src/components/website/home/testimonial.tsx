...
import { userTestimonialStore } from "@/src/store/website/testimonial"
...
//     name: "Sophia Loren",
//     country: "USA",
//     text: "Amazing experience! The web development and design work exceeded my expectations, and I couldn't be happier with the results.",
//     date: "May 9, 2023",
//     time: "10:30 PM",
//     avatar: '/placeholder.svg?height=48&width=48',
//     platform: "Facebook"
...
//     name: "Daniel Garcia",
//     country: "Spain",
//     text: "Fantastic service! Egens Lab's team was efficient, creative, and delivered high-quality work that truly helped my business grow.",
//     date: "May 9, 2023",
//     time: "10:30 PM",
//     avatar: '/placeholder.svg?height=48&width=48',
//     platform: "google"
...
export default function Testimonials() {
  const [activePlatform, setActivePlatform] = useState("tripadvisor")
  const [currentIndex, setCurrentIndex] = useState(0)
  const itemsPerPage = 3

  const { getTestimonialList, testimonialData, isTestimonialLoading } = userTestimonialStore()

  useEffect(() => {
    if (activePlatform) {
      getTestimonialList(activePlatform)
    }
  }, [activePlatform])

  const filteredTestimonials = testimonialData.filter(testimonial => testimonial.type === activePlatform)

  const goToNext = () => {
    if (currentIndex < filteredTestimonials.length - itemsPerPage) {
  ...
  const dateFiltar = (createdAt: string) => {
  ...
  const timeFiltar = (createdAt: string) => {
    const dateObj = new Date(createdAt);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center font-secound gap-3 mb-3">
            ...
            <span className="text-[#b0191e] text-base"> Testimonial </span>
            <span className="text-xl">
              <svg xmlns="http://www.w3.org/2000/svg" width="15" height="16" viewBox="0 0 15 16" fill="#b0191e">
                <path
                  fill-rule="evenodd"
        ...
        <div className="relative">
          {filteredTestimonials.length >= 3 ? (
            <>
              <button
                className="absolute left-[-20px] top-1/2 transform -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gray-600 hover:bg-gray-50"
                onClick={goToPrevious}
              >
                <ChevronLeft />
              </button>
              <button
                className="absolute right-[-20px] top-1/2 transform -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gray-600 hover:bg-gray-50"
                onClick={goToNext}
              >
                <ChevronRight />
              </button>
          ...
          {/* Testimonial Slider with Platform Filter */}
                ...
                filteredTestimonials.slice(currentIndex, currentIndex + itemsPerPage).map((testimonial, index) => (
                  <div key={index} className=" relative ">
                    ...
                    <div className="flex items-center justify-center gap-2 mt-8">
                      ...
                      <div>
                        <h4 className="font-semibold text-lg text-gray-900 leading-5">{testimonial?.name}</h4>
                        <p className="text-[13px] text-gray-500">{testimonial?.countryName}</p>
                      </div>
                    </div>
